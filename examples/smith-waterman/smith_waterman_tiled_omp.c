...
int main ( int argc, char* argv[] ) {
    ...
    allocated[0] = 0;
    tile_matrix[0][0].bottom_right = allocated;
    ...
    for ( i = 1; i < n_tiles_height + 1; ++i ) {
        ...
        tile_matrix[i][0].right_column = allocated;

        allocated = (int*)malloc(sizeof(int));
        allocated[0] = GAP_PENALTY*(i*tile_height); //sagnak: needed to handle tilesize 2
        tile_matrix[i][0].bottom_right = allocated;
    }
        ...
        for ( i = ((0 > j-n_tiles_width) ? 0 : j-n_tiles_width+1); i < ((j < n_tiles_width) ? j+1 : n_tiles_height); ++i ) {
            ...
            for ( ii = 1; ii < tile_height+1; ++ii ) {
                for ( jj = 1; jj < tile_width+1; ++jj ) {
                    signed char char_from_1 = string_1[(j-i+1-1)*tile_width+(jj-1)];
                    signed char char_from_2 = string_2[(i+1-1)*tile_height+(ii-1)];
            ...
            tile_matrix[i+1][j-i+1].bottom_right = curr_bottom_right;
            ...
            tile_matrix[i+1][j-i+1].right_column = curr_right_column;
            ...
            tile_matrix[i+1][j-i+1].bottom_row = curr_bottom_row;
    ...
    int score = tile_matrix[n_tiles_height][n_tiles_width].bottom_row[tile_width-1];
