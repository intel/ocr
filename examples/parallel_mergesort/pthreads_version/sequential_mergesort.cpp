...
void sequential_mergesort(int *sort_array, int *temp_array, int start_index, int end_index){
  int i, j, k, new_chunk_size, j_start;
  ...
  new_chunk_size = ((end_index+1) -start_index)/2;  /* +1 because of
						       zero-indexed arrays */
  sequential_mergesort(sort_array, temp_array, start_index, (start_index+new_chunk_size)-1);
  sequential_mergesort(sort_array, temp_array, start_index + new_chunk_size, end_index);
